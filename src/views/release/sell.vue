...
<div class="box">
  <div class="release-wrap">
    <div class="top-title">
      <img src="../../assets/images/back_icon.png" alt="" @click="back_page_prev">销售挂牌
    </div>
    <group class="group-wrap">
      <cell-box class="cell-item cell2-item" @click.native="clickSelect(1)">
        <span class="w-label">
          <span class="dot-tip">*</span>挂牌方式</span>
        <span class="value-sp">{{form.brandMode}}</span>
      ...
      <x-input class="cell-item" placeholder="请输入" v-model="form.brandTotal" type='number'>
        <span slot="label" class="w-label">
          <span class="dot-tip">*</span>挂牌总数量</span>
        <span slot="right">吨</span>
      </x-input>
      <x-input class="cell-item" placeholder="请输入" v-model="form.minTotal" type='number'>
        <span slot="label" class="w-label">
      ...
      <cell-box v-show="form.brandModeId == 2" class="cell-item cell2-item" @click.native="uploadFile">
        <span class="w-label">
          <span class="dot-tip">*</span>仓单扫描件</span>
          <span class="value-sp">选择文件</span>
          <input type="file" style="display:none;" ref="files">  
      </cell-box>
  ...
  methods: {
    ...
    onChange(val) {
      if (this.flagSel == 1) {
        ...
        this.form.brandMode = _sel.name;
        this.form.brandModeId = _sel.value;
      ...
      getCoalType()
        .then(({ data }) => {
          ...
          data.forEach(element => {
            let cell = {};
            (cell.name = element.Name), (cell.value = element.Id);
            list.push(cell);
          });
          this.coal_type = list;
        })
